...
function [feat_disease seg_img] =  EvaluateFeatures_Grayscale(I)
...
% subplot(2,4,5);imshow(segmented_images{4});title('Cluster 4');
% subplot(2,4,6);imshow(segmented_images{5});title('Cluster 5');


% Feature Extraction
x = inputdlg('Enter the cluster no. containing the disease affected brain part only:');
i = str2double(x);
...
cc = bwconncomp(seg_img,6); %finds the 6-conneced components in the image
diseasedata = regionprops(cc,'basic'); % here basic refers to basic properties of an image
%stats = regionprops(CC,properties) measures a set of properties for each connected component (object) in CC,
